...
class Thumbs extends Component {
    ...
    updateSizes = () => {
        ...
        this.itemSize = this.props.thumbWidth
            ? this.props.thumbWidth
            : outerWidth(this.refs.thumb0);
    ...
    getImages() {
        const images = React.Children.map(
            ...
            (item, index) => {
                ...
                if (item.type !== "img") {
                    img = React.Children.toArray(item.props.children).filter(
    ...
    renderItems() {
        return this.state.images.map((img, index) => {
            const itemClass = klass.ITEM(
                false,
                index === this.state.selectedItem && this.state.hasMount
            );
    ...
    render() {
        ...
        // show right arrow
        const hasNext =
            this.showArrows && this.state.firstItem < this.lastPosition;
